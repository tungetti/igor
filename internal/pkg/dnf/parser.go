...
// parseDnfInfo parses dnf info output.
// Returns package information for the first package found.
func parseDnfInfo(output string) (*pkg.Package, error) {
...
// parseDnfSearch parses dnf search output.
// Format varies but typically:
// package-name.arch : Description
// or with summary sections
func parseDnfSearch(output string) ([]pkg.Package, error) {
	var packages []pkg.Package
...
// parseDnfList parses dnf list output.
// Format: package-name.arch    version    repository
func parseDnfList(output string) ([]pkg.Package, error) {
	var packages []pkg.Package
...
// parseDnfRepolist parses dnf repolist --all output.
// Format: repo-id              repo-name                                status
func parseDnfRepolist(output string) ([]pkg.Repository, error) {
	...
	for _, line := range lines {
		...
		// Skip header line (contains "repo id" or similar)
		if strings.Contains(strings.ToLower(line), "repo id") ||
			strings.HasPrefix(line, "Last metadata expiration") {
			headerPassed = true
			continue
		}
		...
		// Format can be:
		// repo-id              repo-name                                enabled
		// repo-id              repo-name                                disabled
		fields := strings.Fields(line)
		if len(fields) < 2 {
			continue
		}
		...
		// The name is everything between ID and status
		name := repoID
		if len(fields) > 2 {
			// Join middle fields as name
		...
		// Use the name if we extracted one different from ID
		if name != repoID && name != "" {
			repos[len(repos)-1].Name = repoID // Keep ID as name for consistency
		}
	}
...
// parseDnfCheckUpdate parses dnf check-update output.
// Format: package-name.arch    version    repository
func parseDnfCheckUpdate(output string) ([]pkg.Package, error) {
	var packages []pkg.Package
