...
// Context holds the installation context and shared state.
// It provides access to system information, package management, and execution
// capabilities needed during installation.
type Context struct {
	// System information
	GPUInfo    *gpu.GPUInfo
...
// NewContext creates a new installation context with the given options.
...
// WithGPUInfo sets the GPU information in the context.
func WithGPUInfo(gpuInfo *gpu.GPUInfo) ContextOption {
	return func(c *Context) {
		c.GPUInfo = gpuInfo
	}
}

// WithDistroInfo sets the distribution information in the context.
func WithDistroInfo(info *distro.Distribution) ContextOption {
	return func(c *Context) {
		c.DistroInfo = info
	}
}

// WithDriverVersion sets the driver version in the context.
func WithDriverVersion(version string) ContextOption {
	return func(c *Context) {
		c.DriverVersion = version
	}
}

// WithComponents sets the components to install in the context.
func WithComponents(components []string) ContextOption {
	return func(c *Context) {
		c.Components = append([]string{}, components...)
	}
}

// WithPackageManager sets the package manager in the context.
func WithPackageManager(pm pkg.Manager) ContextOption {
	return func(c *Context) {
		c.PackageManager = pm
	}
}

// WithExecutor sets the command executor in the context.
func WithExecutor(executor exec.Executor) ContextOption {
	return func(c *Context) {
		c.Executor = executor
	}
}

// WithPrivilege sets the privilege handler in the context.
func WithPrivilege(priv *privilege.Manager) ContextOption {
	return func(c *Context) {
		c.Privilege = priv
	}
}
...
// WithDryRun sets the dry run mode in the context.
...
// WithContext sets a parent context for cancellation.
